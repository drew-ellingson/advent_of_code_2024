...
@lru_cache
def count_ways_one(available: Tuple[str, ...], pattern: str) -> Optional[int]:
    if not pattern:
        return 1
    else:
        cands = [pattern.replace(a, "", 1) for a in available if pattern.startswith(a)]
        return sum(count_ways_one(available, c) or 0 for c in cands)


def count_ways_all(
    available: Tuple[str, ...], patterns: Tuple[str, ...], p2: bool = False
) -> int:
    counts = [count_ways_one(available, p) or 0 for p in patterns]
